...
int main(int argc, char *argv[]) {
    ...
    int outfile_status1 = 0, outfile_status2 = 0, outfile_status3 = 0;
    ...
    if (p_rank == 0) {
        ...
        char r_file_name[1024] = "";
        strcat(r_file_name,result_fileName);
        strcat(r_file_name,".dat");

        char r_fail_file_name[1024] = "";
        strcat(r_fail_file_name,result_fileName);
        strcat(r_fail_file_name,"_fail_stat.dat");

        char r_drift_file_name[1024] = "";
        strcat(r_drift_file_name,result_fileName);
        strcat(r_drift_file_name,"_drift.dat");
        ...
        if ((outf_drift = fopen(r_drift_file_name, "r")) == NULL ) { /* File doesn't exist */
            if ((outf_drift = fopen(r_drift_file_name, "w")) == NULL ) {
                fprintf(stderr, "\nERROR: Can't open %s for output.\n\n", r_drift_file_name);
                outfile_status3 = 999;
            }
        } else {
    ...
    MPI_Bcast(&outfile_status3, 1, MPI_INT, 0, MPI_COMM_WORLD );
...
//==============================================================================
    ...
    cmg_state = malloc(sizeof(CMG_one_orbit_state));
    CMG_one_orbit_state_init( cmg_state, startTime );
#else
...
#if defined(_USE_CMG_ONE_ORBIT_STATE_)
                        CMG_one_orbit_state_reset( cmg_state, curTime, 0, p_rank);
#else
                        free(cmg_list);
                        cmg_list = (struct CMG_List*)malloc(sizeof(struct CMG_List));
                        initCMGList(cmg_list);
...
#if defined(_USE_CMG_ONE_ORBIT_STATE_)
            cmgtotal[cnt_good]  = cmg_state->cmg_total;
#else
