// 这个版本基于SearchNewPoints_new3.c,取消了将望远镜往天顶或者其运动方向引导的部分。
// 取而代之的是当CMG条件不满足时，将大角度转动拆分为5度、5度+。。。的转动。
...
//  计算sky_id_tracker[]的指标范围
    ...
    if ( isInSunSide == 1 ) {// 在阳照区时
        ...
        for( i=idx_start; i<idx_end; i++ ){
            ...
            value_sun_angle_to_nomal = fabs(  sun[0]*skyMap[s_id].nx 
            ...
            //  判断是否可以通过多次5度的转动来完成指向的改变
                // try5degs_works = (int)( ((curTime - *lastObsEndTime)*86400+tTime)/90 ) * 5. >= tAngle;
                // if( try5degs_works ){
                //     cmg_use = 0;
                // }
                ...
                if( try_max_steps > 0 ){
                    ...
                    while( steps <= try_max_steps ){
                        ...
                        if( tTime_tmp <= t_left ){
                            // print_Error_Msg("--> smart search done!",0);
                            try5degs_works = true;
                            cmg_use = get_cmg_use(angle_left);
                            tTime = steps*90 + tTime_tmp; // dont forget to udate tTime!
                            try_steps++;
                            break;
                        }
                        steps += 1;
...
// 确定曝光时间
                ...
                if(angleValue > 1) { // 跟暗边的夹角，不需要考虑增加曝光时间来使得信噪比达到要求
...
// =============================================================================
            ...
            //  CMG 条件判断
            /////////////////
                // double cmg_times = 0;  // 当前队列对应的时间长度
...
/************************************************************************/
        ...
        //  判断是否可以通过多次5度的转动来完成指向的改变
            // try5degs_works = (int)( ((curTime - *lastObsEndTime)*86400+tTime)/90 ) * 5. >= tAngle;
            // if( try5degs_works ){
            //     // print_Error_Msg("try5degs_works == 1, CMG will not be affected, so we set cmg_use = 0!",0);
            //     cmg_use = 0;
            // }
...
/************************************************************************/
    ...
    MPI_Bcast(&final_try_steps, 1, MPI_INT, final_result.n_rank, MPI_COMM_WORLD);
    ...
    if ( final_min_value >= MAX_VALUE ) {
        ...
        // lastObsEndTime 用于记录上一次观测结束的时刻，以便在下一次的搜索中使用。
        *lastObsEndTime = returnTime;
    ...
    //  跑到了阳照区，于是这里重新判断一下是否在阳照区，并且重新计算帆板法线与太阳的夹角
        ...
        *trysteps = final_try_steps;
        Update_CMGList( cmg_list, 
                        otime,
                        final_cmg_value,
                        final_use_time );

        if ( cmg_list->cmg_total > CMG_THRES ) {
            print_Error_Msg("CMG is in danger",0);
        }
        ...
        if ( battery_q < BATTERY_LOW ) {
            char msg[1024];
            sprintf(msg,"Energy not balance, Position 2, battery is %f",battery_q);
            print_Error_Msg(msg,0);
            MPI_Finalize();
            exit(0);
        }
