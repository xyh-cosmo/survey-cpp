// 这个版本基于SearchNewPoints_new.c,但是策略部分有更大的改动
...
//              根据太阳和帆板法线的夹角来修正权重，尽量观测小角度的天区
                ...
                if( (curTime-2459766)/365.25 <  HIGH_LATITUDE_PRIOR_TIME ) {
                    ...
                    weight += 500*( fabs(skyMap[s_id].gb) <= Galaxy_B_Sec_Low );
                    weight += 500*( fabs(skyMap[s_id].dec) <= Ecliptic_Lat_Sec_Low );
                }
#endif
...
/************************************************************************/
        ...
        //  开始分配权重
            ...
            if( (curTime-2459766)/365.25 <  HIGH_LATITUDE_PRIOR_TIME ) {
                ...
                weight += 500*( fabs(skyMap[i].gb) <= Galaxy_B_Sec_Low );
                weight += 500*( fabs(skyMap[i].dec) <= Ecliptic_Lat_Sec_Low );
            }
#endif
